...
public class Agent {
	...
	private synchronized static KnowledgeBase getKnowledgeBase() throws IllegalArgumentException {
		if (kbaseSingleton==null) {
			...
			kbuilder.add(ResourceFactory.newInputStreamResource(Agent.class.getResourceAsStream("fly.drl")), ResourceType.DRL);
	...
	private String createGame() throws MalformedURLException, IOException {
		String id = post("{email:andre-gd@hotmail.com, seed:"+seed+"}",new URL("http://66.228.62.78:8080/server-0.5-SNAPSHOT/creategame"));
	...
	private Percept percept() throws MalformedURLException {
		String json = getJsonFromServer(new URL("http://66.228.62.78:8080/server-0.5-SNAPSHOT//percept?id="+sessionId));
		logger.debug(json);
	...
	public Percept move(Direction d) throws IOException {
		String json = post("{id:"+sessionId+", move:"+d+"}",new URL("http://66.228.62.78:8080/server-0.5-SNAPSHOT/action"));
		logger.debug(json);
		Percept p = new Gson().fromJson(json, Percept.class);
		return p;
	}
	...
	public Percept shoot(Direction d) throws IOException {
		String json = post("{id:"+sessionId+", fire:"+d+"}",new URL("http://66.228.62.78:8080/server-0.5-SNAPSHOT/action"));
		logger.debug(json);
		Percept p = new Gson().fromJson(json, Percept.class);
		return p;
	}
		...
		String json = post("{id:"+sessionId+", throwRock:"+d+"}",new URL("http://66.228.62.78:8080/server-0.5-SNAPSHOT/action"));
		logger.debug(json);
		Percept p = new Gson().fromJson(json, Percept.class);
		return p;
	}
